...
const componentFolder = './src/app/components/';
...
function parseComponent(component, filename) {
  ...
  const splitIndex = filename.indexOf('/src/app/');
    ...
    list.forEach(file => {
      ...
      fs.stat(file, async (err, stat) => {
        ...
        if (stat && stat.isDirectory()) {
          ...
          if (
            file.endsWith('layout.js') ||
            (file.endsWith('index.js') && !file.endsWith('.story.js') && !file.endsWith('.test.js'))
          ) {
